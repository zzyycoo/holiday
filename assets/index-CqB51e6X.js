...
Thank you`}var F=1;function I(){let t=document.getElementById(`golf-section`);if(!t)return;if(e.selectedServices.has(`room`)){let e=document.createElement(`div`);e.id=`golf-sync-section`,e.innerHTML=`
...
Thank you`}function H(){let t=document.getElementById(`bus-section`);if(!t)return;let n=!e.selectedServices.has(`room`);if(n){let e=document.createElement(`div`);e.id=`bus-agent-section`,e.innerHTML=`
      <div class="form-row" style="margin-bottom: 1rem;">
        <div class="form-group">
          <label>Agent</label>
...
Thank you`}var ue={room:{enabled:!0,icon:`🏨`,title:`Room Booking`},car:{enabled:!0,icon:`🚗`,title:`Car Service`},golf:{enabled:!0,icon:`⛳`,title:`Golf Booking`},bus:{enabled:!0,icon:`🚌`,title:`Bus Service`}};function de(){return ue}var G=`3.1.0`;function fe(){t(),pe(),me(),ye(),window.pidDatabase=new Map,window.oldPIDIndex=new Map,window.newPIDIndex=new Map,window.pidDatabaseLoaded=!1,console.log(`Booking App v${G} initialized`)}function pe(){let e=document.querySelector(`.app-header`);e&&(e.innerHTML=`
      <h1>🏨 Hotel Service Booking System</h1>
      <div class="version">v${G} | Modular Vite Architecture</div>
...
`)),`Subject: ${s}\n\n${c}\n\nAuthorizer: ${r}\n\nThank you`}window.app={selectService:he,setAgent:S,selectHotel:C,addGuestForm:w,removeGuestForm:re,addSharerForm:ie,removeSharerForm:ae,setRoomType:E,setRoomTypeFromSelect:se,searchPID:ce,addCarForm:j,removeCarForm:M,setLocation:N,addGolfGuestForm:L,removeGolfGuestForm:R,updateGolfPax:z,syncGolfGuests:B,toggleBusRoute:U,addA171Patron:$,removeA171Patron:xe,generateAllEmails:K,copyToClipboard:q,sendEmail:J,showToast:Y,handlePIDImport:X,formatDate:d,formatDateShort:f,getTodayStr:p,getTomorrowStr:m,resolvePID:h},document.addEventListener(`DOMContentLoaded`,fe);